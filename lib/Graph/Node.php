...
{
  /**
   * Efficient path caching. This will cache relative path to any destination
   * it has been asked for looking, even if not the first node.
   * 
   * @var array
   */
  ...
  protected function _parseLinks($destination) {
    ...
    foreach ($this->links as $link) {
      ...
      // Do not cache any result when we are recursing into the same
      // node twice.
    ...
    if (!$found && $tainted) {
      // This is not the proper way, I assume. But it ensures that we won't
      // cache false negative in case of a circular dependency track.
